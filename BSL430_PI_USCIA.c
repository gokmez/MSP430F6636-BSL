...
char receiveByte()
{
  while( !(UCA1IFG & UCRXIFG));       // wait for RX flag
  return UCA1RXBUF;
}
/*******************************************************************************
*Function:    verifyData
...
void init_USCI( char baud_rate )
{
  ...
  {
    case BAUD_9600:
      UCA1BRW = 833 ;                           // 8MHz 9600 (see User's Guide)
      UCA1MCTL = UCBRS_2 + UCBRF_0;             // Modulation
    break;
    case BAUD_19200:
      UCA1BRW = 416;                            // 8MHz 19200 (see User's Guide)
      UCA1MCTL = UCBRS_6 + UCBRF_0;             // Modulation
    break;
    case BAUD_38400:
      UCA1BRW = 208;                            // 8MHz 38400 (see User's Guide)
      UCA1MCTL = UCBRS_3 + UCBRF_0;             // Modulation
    break;
    case BAUD_57600:
      UCA1BRW = 138;                            // 8MHz 57600 (see User's Guide)
  ...
  USCI_PORT_SEL = RXD + TXD;                    // P8.2-3 = USCI_A1 TXD/RXD
